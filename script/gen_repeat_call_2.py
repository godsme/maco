...
def gen_repeat_call_macro(n):
    all_macros = ""
    for i in range(n):
        all_macros = all_macros + "#define __MACO_repeat_call_2_{}(call, i, x, ...)".format(i)
        if i > 0:
            all_macros = all_macros + " call(i, x) __MACO_repeat_call_2_{}(call, __MACO_succ(i), __VA_ARGS__)".format(i - 1)
